...
from mapper.telemetry import get_tracer, init_telemetry, shutdown_telemetry
...
agent_card = AgentCard(
    ...
    provider=AgentProvider(organization="Srotas Health", url="https://srotas.ai"),
    default_input_modes=["text/plain", "application/json"],
    default_output_modes=["application/json"],
    supported_interfaces=[
        AgentInterface(
            url="/api/v1/a2a/jsonrpc", protocol_binding="JSONRPC", protocol_version="1.0.0"
        ),
        AgentInterface(url="/api/v1/a2a", protocol_binding="HTTP+JSON", protocol_version="1.0.0"),
        AgentInterface(
            url="/api/v1/map-cohort", protocol_binding="http-rest", protocol_version="1.0.0"
        ),
        AgentInterface(
            url="/api/v1/agent/linguist", protocol_binding="http-rest", protocol_version="1.0.0"
        ),
        AgentInterface(
        ...
        AgentInterface(
            url="/api/v1/agent/auditor", protocol_binding="http-rest", protocol_version="1.0.0"
        ),
    ],
    skills=[
        AgentSkill(
    ...
    capabilities=AgentCapabilities(
        extended_agent_card=True, streaming=True, push_notifications=False
    ),
)
...
@app.post("/api/v1/agent/linguist", response_model=ClinicalIntent | None)
...
async def agent_informatician(req: InformaticianRequest, request: Request):
    ...
    with tracer.start_as_current_span("Informatician.retrieve", context=ctx) as span:
        ...
        return InformaticianResponse(candidates=candidates, prog_candidates=prog_candidates)
...
async def agent_auditor(req: AuditorRequest, request: Request):
    ...
    with tracer.start_as_current_span("Auditor.validate_codes", context=ctx) as span:
        ...
        return AuditorResponse(
            is_approved=is_approved, selected=selected, rejected=rejected, critique=critique
        )
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            while not is_approved and attempt < max_retries:
                ...
                if not linguist_resp.is_success:
                    raise HTTPException(
                        status_code=502, detail="Failed communicating with Linguist Agent"
                    )
                ...
                if not informatician_resp.is_success:
                    raise HTTPException(
                        status_code=502, detail="Failed communicating with Informatician Agent"
                    )
                ...
                filtered_out = [
                    c for c in prog_candidates if c.code not in [fc.code for fc in candidates]
                ]
                ...
                if not auditor_resp.is_success:
                    raise HTTPException(
                        status_code=502, detail="Failed communicating with Clinical Auditor Agent"
                    )
            ...
            if selected:
                span.set_attribute(
                    "clinical.top_code", f"{selected[0].vocabulary}:{selected[0].code}"
                )
