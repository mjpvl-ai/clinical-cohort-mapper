...
from mapper.schemas import ClinicalIntent, CandidateCode, SelectedCode, RejectedCandidate
from mapper.db import get_concept_relationships, query_local_concepts
...
class ClinicalAuditor:
    ...
    def audit(self, intent: ClinicalIntent, candidates: List[CandidateCode]) -> Tuple[bool, List[SelectedCode], List[RejectedCandidate], Optional[Dict[str, Any]]]:
        ...
        try:
            ...
            candidates_data = [{"vocabulary": c.vocabulary, "code": c.code, "display": c.display} for c in candidates]
            ...
            for item in data.get('selected_codes', []):
                ...
                if any(c.code == item['code'] for c in candidates):
                    llm_selected.append(SelectedCode(
            ...
            for item in data.get('rejected_candidates', []):
                llm_rejected.append(RejectedCandidate(
                    vocabulary=item['vocabulary'],
                    code=item['code'],
                    display=item['display'],
    ...
    def _rule_based_audit(self, intent: ClinicalIntent, candidates: List[CandidateCode]) -> Tuple[bool, List[SelectedCode], List[RejectedCandidate], Optional[Dict[str, Any]]]:
        ...
        is_ckd_stage_3_query = 'stage 3' in intent.original_query.lower() and 'kidney' in intent.original_query.lower()
        if is_ckd_stage_3_query:
            has_broad_candidate = any(c.code == 'N18.9' for c in candidates)
            has_specific_candidate = any(c.code in ['N18.30', 'N18.31', 'N18.32'] for c in candidates)
        ...
        for cand in candidates:
            ...
            # --- Condition Specific Rules ---
                ...
                if 'diabetes' in intent.original_query.lower():
                    ...
                    if cand.code == 'J44.9':
                        confidence = 0.95
                        reason = "Standard code for Chronic obstructive pulmonary disease, unspecified."
            ...
            # --- Drug Specific Rules ---
                ...
                is_class_query = any(w in intent.original_query.lower() for w in ['agonist', 'corticosteroid', 'inhibitor', 'blocker'])
