...
def get_local_model():
    ...
    gguf_path = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "models", "medgemma-4b-it-Q4_K_M.gguf"
    )
...
def call_ollama(prompt: str, json_mode: bool = False) -> str:
    ...
    try:
        with urllib.request.urlopen(ping_url, timeout=1.0):
            pass
    except Exception as e:
        raise RuntimeError(f"Ollama service is not running at {ping_url}: {e}") from e
...
def call_llm(prompt: str, json_mode: bool = False) -> str:
    ...
    if api_key:
        ...
        try:
            ...
            logger.warning(
                f"Gemini API call failed ({e}). Falling back immediately to local options."
            )
