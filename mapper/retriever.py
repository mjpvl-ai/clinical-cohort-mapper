...
from mapper.schemas import ClinicalIntent, CandidateCode
...
class MedicalInformatician:
    ...
    def retrieve_candidates(self, intent: ClinicalIntent) -> List[CandidateCode]:
        ...
        try:
            ...
            candidates_data = [{"vocabulary": c.vocabulary, "code": c.code, "display": c.display} for c in unique_candidates]
            ...
            for fc in filtered_codes:
                ...
                if k not in seen:
                    ...
                    orig = next((c for c in unique_candidates if c.vocabulary == fc['vocabulary'] and c.code == fc['code']), None)
                    if orig:
                        refined_candidates.append(orig)
                    else:
                        refined_candidates.append(CandidateCode(
    ...
    def _retrieve_drugs(self, intent: ClinicalIntent, seen_codes: set) -> List[CandidateCode]:
        ...
        for entity in intent.clinical_entities:
            ...
            is_class = any(w in entity.lower() for w in ['agonist', 'corticosteroid', 'inhibitor', 'blocker', 'class'])
            ...
            if is_class:
                ...
                if 'corticosteroid' in class_search_term.lower():
                    concepts = [{'classId': 'N0000175576', 'className': 'Corticosteroid', 'classType': 'EPC'}]
                elif 'glp-1' in class_search_term.lower():
                    concepts = [{'classId': 'N0000178480', 'className': 'GLP-1 Receptor Agonist', 'classType': 'EPC'}]
                else:
                    url = f"https://rxnav.nlm.nih.gov/REST/rxclass/class/byName.json?className={urllib.parse.quote(class_search_term)}"
                    data = self._http_get(url)
                    concepts = data.get('rxclassMinConceptList', {}).get('rxclassMinConcept', []) if data else []
                ...
                for c in concepts:
                    ...
                    members = members_data.get('drugMemberGroup', {}).get('drugMember', []) if members_data else []
                ...
                candidates_list = data.get('approximateGroup', {}).get('candidate', []) if data else []
                for c in candidates_list:
                    ...
                    if not name and rxcui:
                        ...
                        name = prop_data.get('properties', {}).get('name', 'Unknown drug') if prop_data else 'Unknown drug'
    ...
    def _retrieve_conditions(self, intent: ClinicalIntent, seen_codes: set) -> List[CandidateCode]:
        ...
        for entity in search_terms:
            ...
            if data and len(data) >= 4:
                ...
                for code, display_arr in zip(codes_list, displays_list):
    ...
    def _retrieve_measurements(self, intent: ClinicalIntent, seen_codes: set) -> List[CandidateCode]:
