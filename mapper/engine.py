...
class MappingEngine:
    ...
    def _build_graph(self) -> StateGraph:
        ...
        def parse_node(state: AgentState) -> dict:
            ...
            with tracer.start_as_current_span(span_name) as span:
                ...
                return {"intent": intent, "attempt": state["attempt"] + 1}

        def retrieve_node(state: AgentState) -> dict:
            tracer = get_tracer()
            with tracer.start_as_current_span("TerminologyClient.retrieve") as span:
                prog_candidates = self.informatician._retrieve_candidates_programmatic(
                    state["intent"]
                )
        ...
        def audit_node(state: AgentState) -> dict:
            ...
            with tracer.start_as_current_span("Auditor.audit") as span:
                is_approved, selected, rejected, critique = self.auditor.audit(
        ...
        workflow.add_conditional_edges("audit", route_next, {"parse": "parse", "end": END})
    ...
    def map_query(self, query: str, max_retries: int = 3) -> MappingResult:
        ...
        with tracer.start_as_current_span("MapClinicalQuery") as span:
            ...
            if not selected and all_candidates:
                ...
                _, selected, fallback_rejected, _ = self.auditor.audit(
                    fallback_intent, all_candidates
                )
    ...
    def _rank_candidates(
        ...
        candidates: list[CandidateCode],
        selected: list[SelectedCode] = None,
    ) -> list[CandidateCode]:
        ...
        final_candidates = []
        for idx, (_, c) in enumerate(scored_candidates, 1):
            c.rank = idx
