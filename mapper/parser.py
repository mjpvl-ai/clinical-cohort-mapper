...
class MedicalLinguist:
    ...
    def __init__(self):
        ...
        self.domain_mappings = {
            ...
            "procedure": ["colonoscopy", "chemotherapy", "chemo", "treated with"],
        }
    ...
    def parse(self, query: str, critique: dict | None = None) -> ClinicalIntent:
    ...
    def _heuristic_parse(self, query: str, critique: dict | None = None) -> ClinicalIntent:
        ...
        if not matched_override:
            ...
            scores = dict.fromkeys(self.domain_mappings.keys(), 0)
        ...
        if "hba1c" in query_lower or "a1c" in query_lower:
            ...
            match = re.search(
                r"(?:with|diagnosed with|taking|on|prescribed)\s+([^><=0-9]+)",
                query_clean,
                re.IGNORECASE,
            )
            entities = [match.group(1).strip()] if match else [query_clean]
            ...
            for ent in entities:
                ...
                for item in db_res:
                    for s in item.get("synonyms", []):
                        if s.lower() not in [x.lower() for x in synonyms] and s.lower() not in [
        ...
        op_match = re.search(
            r"(above|greater than|greater than or equal to|below|less than|less than or equal to|>|>=|<|<=)\s*(\d+(?:\.\d+)?)",
            query_lower,
        )
        if op_match:
            ...
            unit_match = re.search(
                r"\d+(?:\.\d+)?\s*(%|mg/dl|ml/min/1.73m²|ml/min/1.73m2|mmhg|kg/m²|kg/m2)",
                query_lower,
            )
            if unit_match:
        ...
        status = "any"
        if any(
            w in query_lower for w in ["currently", "current", "taking", "prescribed", "active"]
        ):
            status = "current"
        ...
        if critique:
            ...
            if "suggested_exclusions" in critique:
                ...
                synonyms = [
                    s
                    for s in synonyms
                    if s.lower() not in [x.lower() for x in negative_constraints]
                ]
